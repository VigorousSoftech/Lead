...
#define kHUDDefaultDisplayDuration 10
...
- (id)initWithFrame:(CGRect)frame {
    ...
    if (self) {
		...
		topLabel = [[UILabel alloc] initWithFrame:CGRectMake(0, offset/3.0, frame.size.width, offset)];
		topLabel.backgroundColor=[UIColor clearColor];
		topLabel.textColor=[UIColor whiteColor];
		topLabel.font=[UIFont fontWithName:@"HelveticaNeue" size:15.0];
		...
		imageView = [[UIImageView alloc] initWithFrame:CGRectMake(frame.size.width/4.0, frame.size.height/2.0,frame.size.width/2.0, frame.size.height/3.0)];
...
+(LGViewHUD*) defaultHUD {
	if (defaultHUD==nil)
		defaultHUD=[[LGViewHUD alloc] initWithFrame:CGRectMake(0, 0, 160, 80)];
	return defaultHUD;
}
...
-(void) setActivityIndicatorOn:(BOOL)isOn {
	if (activityIndicatorOn!=isOn) {
		...
		if (activityIndicatorOn) {
			...
			activityIndicator.center=CGPointMake(self.bounds.size.width/2.0, self.bounds.size.height/2.0+5);
